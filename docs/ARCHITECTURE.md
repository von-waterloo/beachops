...
`POST /api/prompts` (ask/plan/do, опционально `images[]` base64 скрины), `GET /api/jobs/{id}/stream` (транскрипт
`beachops_run_events`). Cloud worker пишет throttled `run.progress`; голос шлёт
`job.progress` captions и (если `VOICE_MILESTONE_TTS`) mid-run TTS-вехи
(`audio.started` kind=`milestone`), затем финальный брифинг (`kind=final`)
с пересобранным control-room snapshot.
...
По умолчанию `AUTO_APPROVE_PLANS=true`: после plan worker сразу enqueue DO
(без owner-кнопок); `AUTO_APPROVE_PLANS=false` возвращает ручной approve.
...
Redis lock гарантирует один run на actor. После plan при `AUTO_APPROVE_PLANS`
(дефолт) — сразу do-follow-up; иначе `awaiting_approval` и owner-кнопки.
