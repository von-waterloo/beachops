...
| `OWNER_USER_IDS` | — | все решения, `/rollback`; при `AUTO_APPROVE_PLANS=false` — Approve |
| `WHITELIST_USER_IDS`, `ADMIN_USER_IDS` | — | legacy fallback: viewer / owner |
| `AUTO_APPROVE_PLANS` | `true` | после `/plan` сразу enqueue DO, без кнопок владельцу |
...
| `SELF_IMPROVE_ENABLED` | `false` | стартовый дефолт до первого переключения в Mini App (Пульт); пока выкл — обычные репо без требований к allowlist |
...
| `VOICE_MILESTONE_TTS` | `true` | устные вехи mid-run в Mini App (ack + статусы) |
