...
Кратко: приватный репозиторий → CI на GitHub-hosted → **auto-deploy с `main` и `dev`**
...
|---------|----------------|
| Push в **`main`** или **`dev`** | Deploy prod ждёт зелёный CI, затем выкат на host-185 |
...
Runner проверяет, что автоматический SHA из `main` или `dev` продолжает историю текущего
`.deployed_sha`. Если ветка отстала от уже развернутой истории, workflow
завершится **до** миграции и перезапуска контейнеров. Сначала перенесите эту
историю в ветку, которую хотите выкатить, обычным merge/rebase.
