...
Полный онбординг (режимы, Mini App, Windows-агент, свой деплой) —
в [базе знаний](./KNOWLEDGE_BASE.md). Ниже — справочник команд и сценариев.
...
Видимое меню `/` в Telegram — компактное; `/help` и `/mode` работают как алиасы (`/help` = `/start`, `/mode` = `/status`), но не показаны в меню, чтобы не дублировать пункты. Кнопка меню чата снова открывает список команд (не Mini App) — так проще вести задачи текстом, пересылками и голосом. Mini App: `/dashboard`.
...
- **Онбординг и справка:** при первом входе — пошаговое знакомство; кнопка **?**
  и полоска типсов под метриками открывают анимированную справку (режимы,
  Windows, очередь, Passkey, деплой). Полный текст — [KNOWLEDGE_BASE.md](./KNOWLEDGE_BASE.md).
