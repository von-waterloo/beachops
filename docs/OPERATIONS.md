...
См. [SELF_DEPLOY.md](./SELF_DEPLOY.md): push в **dev** → CI →
auto-deploy на runner [self-hosted, host-185]; `main` проходит только CI.
Rollback и ручной деплой —
...
добавляет отсутствующий encryption key/policy compatibility, собирает candidate
и запускает миграции до остановки старого poller. При ошибке migration текущий
bot/API/webapp остаются запущены.

Порядок на сервере:

```bash
docker compose -p tg-cursor-bot build
docker compose -p tg-cursor-bot run --rm --no-deps migrate
docker compose -p tg-cursor-bot up -d --no-deps --force-recreate bot worker api webapp
```

Только после успешной migration старый long-polling процесс завершается до
старта нового (иначе Telegram отдаёт updates двум инстансам → `Conflict`).
