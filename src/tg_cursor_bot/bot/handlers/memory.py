...
from tg_cursor_bot.services.inline_keyboards import memory_keyboard
...
async def memory_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    items = await app.memory.list_recent(
        user.id,
        repo_id=repo.id if repo else None,
    )
    if not items:
    ...
    runs = await app.memory.list_runs_for_retry(user.id)
    lines = [memory_header()]
    lines.extend(memory_item_line(item) for item in items)
    await update.message.reply_text(
        "\n".join(lines),
