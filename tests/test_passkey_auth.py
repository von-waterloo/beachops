...
from beachops.web.telegram_auth import TelegramInitDataError
...
@pytest.mark.asyncio
async def test_resolve_request_falls_back_to_session_when_tma_stale(
    ...
    context = SimpleNamespace(
        redis=FakeRedis(
            {
                _session_key(token): (
                    b'{"userId":42,"authMethod":"telegram","username":"owner"}'
                )
            }
        ),
        settings=SimpleNamespace(
            role_for=lambda user_id: Role.OWNER,
            webapp_base_url="https://beachops.marketolog.tech",
        ),
    )
    ...
    monkeypatch.setattr("beachops.web.passkey_auth._validate_tma", boom)
    ...
    principal = await resolve_request_principal(request, "tma stale-init-data")
